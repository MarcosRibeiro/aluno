...
describe('Deslocamento Management Update Component', () => {
  ...
  describe('ngOnInit', () => {
    it('Should call CadastroAluno query and add missing value', () => {
      const deslocamento: IDeslocamento = { id: 456 };
      const cadastroAluno: ICadastroAluno = { id: 30062 };
      deslocamento.cadastroAluno = cadastroAluno;

      const cadastroAlunoCollection: ICadastroAluno[] = [{ id: 26086 }];
    ...
    it('Should update editForm', () => {
      const deslocamento: IDeslocamento = { id: 456 };
      const cadastroAluno: ICadastroAluno = { id: 6791 };
      deslocamento.cadastroAluno = cadastroAluno;

      activatedRoute.data = of({ deslocamento });
      comp.ngOnInit();

      expect(comp.cadastroAlunosSharedCollection).toContain(cadastroAluno);
