...
import { DeslocamentoService, EntityArrayResponseType } from '../service/deslocamento.service';
...
export class DeslocamentoComponent implements OnInit, OnDestroy {
  private static readonly NOT_SORTABLE_FIELDS_AFTER_SEARCH = ['nome', 'grau'];
  ...
  trackId = (index: number, item: IDeslocamento): number => this.deslocamentoService.getDeslocamentoIdentifier(item);

  search(query: string): void {
    const { predicate } = this.sortState();
    if (query && predicate && DeslocamentoComponent.NOT_SORTABLE_FIELDS_AFTER_SEARCH.includes(predicate)) {
  ...
  delete(deslocamento: IDeslocamento): void {
    const modalRef = this.modalService.open(DeslocamentoDeleteDialogComponent, { size: 'lg', backdrop: 'static' });
    modalRef.componentInstance.deslocamento = deslocamento;
  ...
  protected fillComponentAttributeFromRoute(params: ParamMap, data: Data): void {
    ...
    if (params.has('search') && params.get('search') !== '') {
      ...
      const { predicate } = this.sortState();
      if (predicate && DeslocamentoComponent.NOT_SORTABLE_FIELDS_AFTER_SEARCH.includes(predicate)) {
        this.sortState.set({ predicate: 'id', order: 'asc' });
      }
    }
  }
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.deslocamentos = dataFromBody;
  }

  protected fillComponentAttributesFromResponseBody(data: IDeslocamento[] | null): IDeslocamento[] {
    return data ?? [];
  }
  ...
  protected queryBackend(page?: number, sortState?: SortState, currentSearch?: string): Observable<EntityArrayResponseType> {
    ...
    return this.deslocamentoService.query(queryObject).pipe(tap(() => (this.isLoading = false)));
  }
